...
### D1. sloff は buf 専用 resolver / preflight checker を持たない

`internal/sloff/toolresolver/buf` も `internal/sloff/preflight/buf` も導入しない。 spec の `tools:` にも `buf:` 形式の declared 種別を追加しない。 buf を sloff から使うときは、 既存の汎用プリミティブ (`exec` resolver と spec.inputs glob) のみで完結させる。
...
PR #8 の review で「v1 buf.lock の dep identity フィールド」「v1 buf.gen.yaml schema」 等の互換性論点が複数浮上した。 これらは「sloff が buf schema の特定 version を理解する」という前提を持った瞬間に生じる API surface の負債。 buf を special-case しないことで、 これら全てを利用者側 ( buf 自身が schema migration を提供している) に委ねられる。
...
- sloff 本体に buf 固有の知識が入らず、 buf schema 変更の追従コストがゼロ
...
- v1 / v2 schema 互換性議論が sloff から完全に外れる
...
`:latest` 利用者が想定外に多い / silent stale 事故が頻発する、 等の状況が生じた場合、 別 ADR で「opt-in の buf-pinned-lint preflight」 を再導入する余地はある。 ただし default で有効にはせず、 利用者が明示的に opt-in する形 ( `SLOFF_BUF_PINNED_LINT=1` 等) を推奨する。

...
[ADR-0003](./0003-fingerprint-storage-strategy.md) で record を git per-task per-input ファイル方式で配置することを確定し、 [Design Doc](../design/architecture.md) で具体的な YAML schema と deterministic ordering 規約を定めた。 初版は `goccy/go-yaml` による YAML 直列化で実装されている。

YAML 直列化は「中身を `cat` / エディタで読める」 という debug 体験の利点がある一方、 監視している (= 利用者を獲得する) 前段階で 2 つの構造的な摩擦が見えてきた:

1. **検索結果のノイズ**: record に含まれる `output.files` の path や hash は、 利用者の `rg` / `ag` / `git grep` / `grep`、 VSCode / IntelliJ の workspace search、 さらに各種 file indexer で軒並みヒットする。 緩和には全ツール / 全エディタで `.sloff/fingerprints/` を ignore 設定する必要があり、 **チーム × ツール数 で配布コストが増える**。 PR diff の `linguist-generated` は git レイヤの解決で、 ローカル検索ツールには効かない
...
サイズ削減・ 後方互換性は本 ADR の主たる動機ではない。 サイズは [ADR-0003 §131](./0003-fingerprint-storage-strategy.md) の試算で問題化していないし、 schema 進化は現行 `schema_version` フィールドで原理的には足りる。
...
- **debuggability**: fingerprint 不一致を疑った時に中身を確認する手段が確保されている
...
- proto package が将来の S3 backend / 組織横断 fingerprint 共有でそのまま流用できる

👎 **Cons**

- `proto.Marshal` のデフォルト挙動は決定論的ではない。 byte stability を保つために運用規約 ( 後述) が必要
- inspect 用のサブコマンド ( `sloff fingerprint show`) を必須機能として追加する必要がある
...
- **proto package を `sloff.fingerprint.v1` に分離する**。 これにより top-level message を `Record` / `Spec` / `Input` / `Output` / `FileEntry` / `ResolvedVersion` の素直な名前で宣言できる ( package qualifier `fingerprintv1.Spec` 等で曖昧さは解消される)。 親 `sloff.v1` 配下に置くと `sloff.v1.Spec` のように fingerprint 文脈が消えて spec config 等と区別がつかない
- **Go 側で alias は使わず、 generated 型を直接引き回す**。 `fingerprint` package は Storage interface / Marshal-Unmarshal helper / Sort / FilePaths / SchemaVersion 定数 / FileExt 定数のみを export し、 record 型は `fingerprintv1.Record` を呼び出し側で直接使う
...
- **proto package 名に version segment を含める** (`sloff.fingerprint.v1`)。 wire-incompatible な変更が必要になった時は新 package (`sloff.fingerprint.v2`) を切る運用とし、 既存 v1 reader を壊さない
...
5. **proto runtime の major upgrade は `schema_version` bump = fingerprint 全 invalidate のイベント** として扱う。 minor / patch upgrade で起きうる micro byte drift は、 (4) の write-skip ルールで git に届かない (semantic 同一なら overwrite しない) 前提で運用する
...
- 「 wire-breaking な変更が必要になった場合」 は package を `sloff.fingerprint.v2` に切る (= 新 reader/writer を実装し、 v1 を deprecated として一定期間並走させる) 運用ルールを併記する

### debug 経路

binary 化で失う inspect 容易性を、 同 PR で以下のサブコマンドで置換する ( 本 ADR の **必須 follow-up**):

- `sloff fingerprint show <path>`: proto record を YAML / JSON にデコード出力 (人間可読)
- `sloff fingerprint diff <path-a> <path-b>`: 2 record の semantic diff ( どの field が違うか)

加えて、 利用者向けの設定例として `git config diff.sloff-fingerprint.textconv "sloff fingerprint show"` の運用を docs に併記する ( ローカル `git diff` で decode 結果が見える)。
...
`.yml` → `.pb` に変更する。 `linguist-generated` 設定は `.sloff/fingerprints/**` 配下の `*.pb` 全体を対象にできるよう `.gitattributes` を更新する。

> **Update (ADR-0010)**: 本 ADR 確定時の filename は `<input_hash>.pb` だったが、 [ADR-0010](./0010-fingerprint-filename-timestamp-prefix.md) で `<initial_creation_timestamp>-<input_hash>.pb` に拡張された。 拡張子 `.pb` は不変。
...
- proto package は S3 backend / 組織横断 fingerprint 共有 / 将来の RPC 化で再利用できる資産になる

### 負の影響

- inspect 用サブコマンド (`sloff fingerprint show` / `sloff fingerprint diff`) を必須機能として追加する必要がある ( 本 ADR と同 PR スコープ)
...
1. **proto schema 配置**: `proto/sloff/fingerprint/v1/fingerprint.proto` に SSoT、 `internal/proto/sloff/fingerprint/v1/fingerprint.pb.go` ( package `fingerprintv1`) に generated code。 buf の `PACKAGE_DIRECTORY_MATCH` lint を満たすため out path も proto package 階層をミラーする
...
5. **`internal/sloff/fingerprint/record.go` の置換**: `Marshal` / `Unmarshal` を proto 経由に切替、 `proto.MarshalOptions{Deterministic: true}` を単一の helper に集約
6. **`internal/sloff/runner` の write-skip ルール**: 既存 record load → semantic 同一性チェック → 必要時のみ write の経路を実装
7. **`sloff fingerprint show` / `sloff fingerprint diff` サブコマンド**: cobra コマンドとして `cmd/sloff/` に追加
...
9. **`docs/design/architecture.md` の更新**:
    - 「fingerprint schema」 節を proto schema にリライト

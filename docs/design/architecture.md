...
- `group: true` を宣言した task は **集約専用ノード ( barrier / alias)** になる ( [ADR-0016](../adr/0016-group-tasks.md))。 `depends` のみを持ち、 `cmd` / `inputs` / `outputs` / `tools` の宣言は load 時 error ( 空 `depends` も error)。 実行も fingerprint も持たず、 「 この task 集合の完了」 を 1 つの名前で下流に提供する ( 後述「タスク間依存」の group 節)
...
fan-in の大きい task 集合 (「 全 per-dir 生成の完了」 など) を待つ consumer が、 N 本のエッジを列挙する代わりに 1 つの名前へ depends できるよう、 `group: true` の集約専用 task を提供する ( [ADR-0016](../adr/0016-group-tasks.md)。 Ninja の `phony`、 Bazel の `alias` / `test_suite` に相当):

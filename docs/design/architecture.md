...
- `group: true` を宣言した task は **集約専用ノード ( barrier / alias)** になる ( [ADR-0017](../adr/0017-group-tasks.md))。 `depends` のみを持ち、 `cmd` / `inputs` / `outputs` / `tools` の宣言は load 時 error ( 空 `depends` も error)。 実行も fingerprint も持たず、 「 この task 集合の完了」 を 1 つの名前で下流に提供する ( 後述「タスク間依存」の group 節)
...
「 この task 集合の完了」 を 1 つの名前で表す集約専用 task を `group: true` で宣言できる ( [ADR-0017](../adr/0017-group-tasks.md)。 Ninja の `phony`、 Bazel の `alias` / `test_suite` に相当)。 上流を **読まずに完了だけを待つ** barrier を、 データ依存エッジの偽装なしで書くための語彙である ( 生成物を実際に読む consumer の fan-out は前述の depends パターンが解であり、 両者は直交する):
...
- **実行モデル**: group は実行されず fingerprint も持たない。 スケジューラ上は「 全 depends の完了で即完了」 するノードで、 depends のいずれかが fail すれば group も fail する ( 下流に伝播)。 RUN / SKIP ログも record も producedBy 登録も無い
- **パターンとの合成**: group の `depends` にも depends パターン ( ADR-0016) を書ける。 パターンは group でも通常 task と同じ経路 ( provider 展開後・depgraph 構築前) で展開されるため、 provider ( ADR-0015) が動的に emit する task 群を 1 つの名前で束ねる barrier は group × パターンで表現する
- **検証との関係**: group は inputs を持たないため inputs 漏れ warning の判定対象にならない ( 判定を弱めるのではなく、 判定すべきものが構造的に無い)。 逆に **group への depends はデータ読み取りの免罪符にならない**: group メンバーの生成物を読む consumer には、 実 producer への直接エッジを depends 漏れ error が従来通り要求する ( group を透過扱いすると「 大きな group に depends すれば何を読んでも通る」 抜け道になるため)
- **invalidate は group を流れない**: depends が input_hash に入らない ( ADR-0013 D4) 帰結として、 group 経由では fingerprint は invalidate されない。 データを読む consumer は直接エッジ + inputs 宣言が必要で、 それは上記 error が強制する
- `sloff graph` では group を実 task と区別できる形 ( Mermaid: hexagon `{{...}}` / DOT: `shape=hexagon`) で表示する

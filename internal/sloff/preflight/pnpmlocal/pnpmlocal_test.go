...
// the shared SLOFF_ALLOW_STALE_DEPS read-only fall-through uniformly with
...
// rather than a hard error, so SLOFF_ALLOW_STALE_DEPS still applies.
...
// runner doesn't silently degrade it via SLOFF_ALLOW_STALE_DEPS.

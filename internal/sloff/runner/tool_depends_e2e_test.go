...
// TestRunner_ToolDepends_PnpmLocalColdBootstrap is the ADR-0019 regression
// guard for the pnpm-local resolver's deferred-capability: a pnpm-local tool
// whose workspace package.json is absent at run start (because the depends
// task has not yet run) must complete the cold bootstrap successfully.
//
// Before the fix, pnpmlocal latched per-package errors via sync.Once and
// froze the workspace snapshot at first load, so the D4 deferred retry could
// never observe the package.json generated by the depends task and always
// returned ErrNotWorkspacePackage.
func TestRunner_ToolDepends_PnpmLocalColdBootstrap(t *testing.T) {
	requireSh(t)
	runE2E(
		t, "tooldepends-pnpmlocal-cold-bootstrap",
		runStep(
			expectWarn("deferred until its declared depends complete"),
			expectInfo(`resolved tool "codegen" after its declared depends completed`),
		),
		runStep(expectNoWarns(), expectNoInfoContaining("RUN ")),
	)
}

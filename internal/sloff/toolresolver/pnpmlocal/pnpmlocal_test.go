...
// TestResolver_ErrorNotLatched guards ADR-0019 D4: a per-package computation
// that fails on the first call must not latch the error — a second call for
// the same package must re-attempt and succeed when the underlying state has
// changed. Without this, the deferred D4 retry can never succeed for a
// pnpm-local tool whose workspace package is generated by a depends task.
func TestResolver_ErrorNotLatched(t *testing.T) {
	root := t.TempDir()

	// First call: workspace exists (lockfile present) but package.json is
	// missing, so the package cannot be found. This simulates the cold-tree
	// state where the depends task has not yet generated the package.
	...
	if _, err := r.Inputs(context.Background(), ".", declared); err == nil {
		t.Fatal("first Inputs call: expected error when package.json is absent")
	}

	// Simulate the depends task running: create the package.json so that the
	// workspace now contains the package.
	mustWriteFile(t, filepath.Join(root, "packages", "codegen", "package.json"), `{"name": "@org/codegen"}`)

	// Second call: must succeed (error must not have been latched).
	got, err := r.Inputs(context.Background(), ".", declared)
	if err != nil {
		t.Fatalf("second Inputs call: expected success after package.json appeared, got %v", err)
	}
	want := []string{"packages/codegen/src/cli.ts"}
	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("Inputs after retry (-want +got):\n%s", diff)
	}
}

// TestResolver_WorkspaceReloadOnPackageMiss guards ADR-0019 D4: when a package
// is absent from the initial workspace snapshot but its package.json appears on
// disk after the depends task runs, a subsequent lookup must reload the
// workspace once and discover the new package. The happy-path (package present
// from the first load) must not pay any extra cost.
func TestResolver_WorkspaceReloadOnPackageMiss(t *testing.T) {
	root := t.TempDir()

	// Workspace contains @org/existing from the start; @org/generated will
	// appear after the first workspace load.
	lockYaml := "lockfileVersion: '9.0'\nimporters:\n  packages/existing: {}\n  packages/generated: {}\n"
	mustWriteFile(t, filepath.Join(root, "pnpm-lock.yaml"), lockYaml)
	mustWriteFile(t, filepath.Join(root, "packages", "existing", "package.json"), `{"name": "@org/existing"}`)
	// packages/generated/package.json is intentionally absent yet.

	stub := &fakeEnumerator{files: map[string][]string{
		filepath.Join("packages", "existing"):  {"packages/existing/index.ts"},
		filepath.Join("packages", "generated"): {"packages/generated/index.ts"},
	}}
	r := mustNewResolver(t, root, stub)

	// Warm up the workspace cache by resolving the present package.
	if _, err := r.Inputs(context.Background(), ".",
		&toolresolver.DeclaredTool{Resolver: "pnpm-local", PackageName: "@org/existing"}); err != nil {
		t.Fatalf("Inputs(@org/existing): %v", err)
	}

	// @org/generated is not yet visible in the cached snapshot.
	if _, err := r.Inputs(context.Background(), ".",
		&toolresolver.DeclaredTool{Resolver: "pnpm-local", PackageName: "@org/generated"}); err == nil {
		t.Fatal("expected error for @org/generated before its package.json exists")
	}

	// The depends task generates the package.json.
	mustWriteFile(t, filepath.Join(root, "packages", "generated", "package.json"), `{"name": "@org/generated"}`)

	// The retry must observe the new package via a workspace reload.
	got, err := r.Inputs(context.Background(), ".",
		&toolresolver.DeclaredTool{Resolver: "pnpm-local", PackageName: "@org/generated"})
	if err != nil {
		t.Fatalf("Inputs(@org/generated) after reload: %v", err)
	}
	want := []string{"packages/generated/index.ts"}
	if diff := cmp.Diff(want, got); diff != "" {
		t.Errorf("Inputs after workspace reload (-want +got):\n%s", diff)
	}
}
